...
.text
    main:
        ...
        la $s2, nota1  # Registrador $s2 contém o endereço da variável nota1
    ...
    calculate_student_average_loop:
        ...
        add.s $s10, $s10, $f0  # Soma a nota atual à soma das notas do aluno atual
        addi $s11, $s11, 1  # Incrementa o contador de notas
        j calculate_student_average_loop  # Volta para o início do loop interno

    check_approval:
        div.s $f1, $s10, 3.0  # Calcula a média do aluno atual
        c.lt.s $f1, 6.0, student_reproved  # Se a média for menor que 6.0, vá para o aluno reprovado
        addi $s7, $s7, 1  # Incrementa o contador de alunos aprovados
        j student_finished  # Vá para o próximo aluno

    student_reproved:
        addi $s8, $s8, 1  # Incrementa o contador de alunos reprovados

    student_finished:
        add.s $s6, $s6, $f1  # Adiciona a média do aluno atual à média da turma
        addi $s9, $s9, 1  # Incrementa o contador de alunos
        j calculate_averages_loop  # Volta para o início do loop externo

    print_results:
        div $s6, $s0  # Calcula a média da turma dividindo a soma das médias dos alunos pelo número de alunos
        print_str("Média da turma: ")
        print_float($f0)  # Imprime a média da turma
        print_str("\n")
        print_str(aprovados)
        print_int($s7)  # Imprime o número de alunos aprovados
        print_str("\n")
        print_str(reprovados)
        print_int($s8)  # Imprime o número de alunos reprovados
        print_str("\n")

        terminate  # Encerra o programa
