...
.data
	Mat: .space 36 # 3*3 * 4 (inteiro)
	
.text
	main:
	li $a1, 3
	la $a0, Mat # Endereco base de Matriz
	...
	li $s0, 0 # Soma dos valores na diagonal secundaria
	...
	e:
		jal indice # Calcula o endere�o de mat[i][j]
  		...
  		move $v0, $a3 # Endere�o base da matriz para retorno
   		...
   		## Neste exemplo a ordem da matriz e de ordem 4, a comparacao sera feita se i+j do 
   		## elemento em questao e igual a 5, se for a soma é realizada, se nao a matriz continua
   		## ate o fim
   		diag_secund:
   			subi $sp, $sp, 4 # Espaco para 1 item na pilha
   			sw $ra, ($sp) # Salva o retorno para a main
		...
		ds:
			jal indice # Calcula o endereco de mat[i][j]
   			lw $a0, ($v0) # Valor em mat[i][j]
   			add $t5, $t0, 1 # Posicao aux de i
   			add $t6, $t1, 1 # Posicao aux de j
   			add $s1, $t5, $t6
   			bne $s1, 5, next
   			add $s0, $s0, $a0
   		next:
